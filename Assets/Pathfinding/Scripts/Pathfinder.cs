...
public class Pathfinder : MonoBehaviour
{
    ...
    // Map size variables
    int startX, startZ, endX, endZ, height, width;
    ...
    void Start()
    {
        fog = FogOfWar.FindExisting(color);
        if (Tilesize <= 0)
        {
            Tilesize = 1;
        }
    ...
    public Pathfinder FindExisting(Player color)
    {
        GameObject obj;
        if (color == Player.RED)
        {
            obj = GameObject.Find("Pathfinder_red");
        }
        else if (color == Player.BLUE)
        {
            obj = GameObject.Find("Pathfinder_blue");
        }
        else
        {
            obj = null;
        ...
        {
            return obj.GetComponent<Pathfinder>();
        }
        return null;
    }
    ...
    public void RedrawMap()
    {
        ...
        for (int i = 0; i < height; i++)
        {
            for (int j = 0; j < width; j++)
            {
                ...
                if (Map[j, i].yCoord == 0)
                {
                    if (fog.IsFoggy(new Vector3(x, 0, y)))
                    {
                        Map[j, i].walkable = false;
                        //It is disallowed, since it is foggy.
                    }
                    else
                    {
                        Map[j, i].walkable = true;
                        //It is allowed, since it is clear.
                    }
                }
            }
        }
