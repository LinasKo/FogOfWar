Shader "Custom/CloudShader"
{
    ...
    SubShader {
        ...
        CGPROGRAM
        #pragma surface DrawClouds Lambert alpha:fade noforwardadd
        ...
        void DrawClouds (Input IN, inout SurfaceOutput o) {
            ...
            /*
             * If you #define SMOOTHSTEP you'll get beacons that react
             * a little more organically, but they aren't as accurate.
             */
...
#else
                half uu = clamp((world_x - CurrentViewport.x) / (CurrentViewport.z - CurrentViewport.x), 0, 1);
                half vv = clamp((world_z - CurrentViewport.y) / (CurrentViewport.w - CurrentViewport.y), 0, 1);
#endif
...
#else
                half uu = clamp((world_x - PreviousViewport.x) / (PreviousViewport.z - PreviousViewport.x), 0, 1);
                half vv = clamp((world_z - PreviousViewport.y) / (PreviousViewport.w - PreviousViewport.y), 0, 1);
#endif
